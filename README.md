# **Hi, I'm Carmela Uccello**

### *Data Scientist & Statistical Analyst*

![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=white)
![R](https://img.shields.io/badge/R-000000?style=for-the-badge&logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-000000?style=for-the-badge&logo=postgresql&logoColor=white)
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-1a1a1a?style=for-the-badge)
![Statistics](https://img.shields.io/badge/Statistical%20Modeling-1a1a1a?style=for-the-badge)

Data Scientist with an academic background in Statistics and practical experience in real-world projects carried out at Istat, Ares 2.0, and Technology Advising. I focus on predictive modeling, quantitative analysis, and the integration of statistical methods with machine learning to support data-driven decisions.

---

## 🔍 **What I Do**
- Statistical modeling and machine learning
- Data cleaning and automated pipeline development
- Exploratory and inferential analysis
- Clear and interpretable data visualizations
- Application of AI to improve statistical processes

---

## 🎓 **Education**
- Bachelor’s Degree in Business and Social Statistics – University of Naples Federico II (108/110)
- Master’s Degree in Statistical Sciences for Decisions – ongoing, expected 2026
- Internships and projects in statistical methods and AI (Istat, Ares 2.0, Technology Advising)

---

## 📈 **Areas of Interest**
- Applied Machine Learning
- Statistical Modeling and Forecasting
- Social Network Analysis
- Data-driven Decision Making
- AI for Statistical Processes

---

## 💻 Technical Skills

I am focused on developing capabilities in **quantitative analysis** and **statistical modeling** through specific programming languages:

### Acquired Certifications and Badges

| Certification | Issued By | Date | Verification |
| :--- | :--- | :--- | :--- |
| **Python for Data Science (Advanced)** | Università Federico II | Feb 2025 | [Verify Here](https://bestr.it/verify/AAOTEXB9NI) |
| **Python for Data Science (Base)** | Università Federico II | Jun 2024 | [Verify Here](https://bestr.it/verify/BNIA5QFJEE) |
| **Celonis Foundations** | Celonis SL | Dec 2024 | [Verify Here](https://www.credly.com/badges/28c657a0-cf29-44ff-8fd2-907e5a092d7e) |
| SAS Programming 1 Essentials | SAS | |  |
| SAS Advanced Analytics | SAS | |  |
| SAS Programming 2 | SAS | |  |

### Key Languages

* **Python:** From beginner to advanced level for Data Science.
* **R & SAS:** Solid foundation for statistical analysis and modeling.
* **SQL:** For database management and querying.

---

## 📬 **Contact**
📧 **melaniauccello37@gmail.com**
